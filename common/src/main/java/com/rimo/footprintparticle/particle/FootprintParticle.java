...
public class FootprintParticle extends SpriteBillboardParticle {
	...
	public void tick() {
		...
		if (this.age > this.maxAge / 2)
			this.colorAlpha -= this.startAlpha / this.maxAge * 2;
	...
	public void buildGeometry(VertexConsumer vertexConsumer, Camera camera, float tickDelta) {
		...
		vertexConsumer.vertex(pos[0].getX(), pos[0].getY(), pos[0].getZ()).texture(this.getMaxU(), this.getMaxV()).color(this.colorRed, this.colorGreen, this.colorBlue, this.colorAlpha).light(this.getBrightness(tickDelta)).next();
		vertexConsumer.vertex(pos[1].getX(), pos[1].getY(), pos[1].getZ()).texture(this.getMaxU(), this.getMinV()).color(this.colorRed, this.colorGreen, this.colorBlue, this.colorAlpha).light(this.getBrightness(tickDelta)).next();
		vertexConsumer.vertex(pos[2].getX(), pos[2].getY(), pos[2].getZ()).texture(this.getMinU(), this.getMinV()).color(this.colorRed, this.colorGreen, this.colorBlue, this.colorAlpha).light(this.getBrightness(tickDelta)).next();
		vertexConsumer.vertex(pos[3].getX(), pos[3].getY(), pos[3].getZ()).texture(this.getMinU(), this.getMaxV()).color(this.colorRed, this.colorGreen, this.colorBlue, this.colorAlpha).light(this.getBrightness(tickDelta)).next();
	}
	...
	public static class DefaultFactory implements ParticleFactory<DefaultParticleType> {
		...
		public Particle createParticle(DefaultParticleType parameters, ClientWorld world, double x, double y, double z, double velocityX, double velocityY, double velocityZ) {
			...
			if (parameters instanceof FootprintParticleType)
				particle.scale *= FPPClient.getEntityScale((((FootprintParticleType)parameters).entity));
			return particle;
		}
	}
